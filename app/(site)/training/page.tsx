...
const TRUST = [
  ...
  ["Access and handling", "Consider who is authorized to access the information, minimum necessary access, data handling, and retention considerations."],
  ["Human review", "Define where facts must be checked, outputs approved, and legal, financial, client, or family-office judgment kept with a responsible person."],
...
export default function TrainingPage() {
  return (
    <>
      <section className="px-6 md:px-8 pt-32 md:pt-44 pb-20 md:pb-28">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-xl md:text-2xl text-on-surface-variant max-w-3xl mb-12 leading-relaxed">
            Aventary trains executives, leadership teams, operators, and privacy-sensitive organizations using their real work—not generic AI demos. Start with the fundamentals, then apply them to the workflows, information, and problems that matter every day.
          </p>
            ...
            {JOURNEY.map((step, i) => (
              <Reveal key={step.num} delay={Math.min(i, 3) * 70}>
                <div className="bg-surface-container-lowest rounded-3xl soft-lift p-8 md:p-12 grid md:grid-cols-[7rem_1fr] gap-6 md:gap-10">
                  <div className="flex md:flex-col items-center md:items-start gap-4"><div className="font-headline text-6xl md:text-7xl font-bold text-primary leading-none">{step.num}</div><div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-primary-container"><span className="material-symbols-outlined text-on-primary-container">{step.icon}</span></div></div>
                  <div><h3 className="font-headline text-2xl md:text-3xl font-bold mb-1">{step.title}</h3><div className="text-on-surface-variant italic font-headline mb-5">{step.sub}</div><p className="text-on-surface-variant leading-relaxed max-w-2xl">{step.body}</p><div className="mt-7 inline-flex items-start gap-2.5 rounded-2xl bg-surface-container-high px-4 py-3"><span className="font-label font-bold text-[11px] tracking-widest uppercase text-accent mt-0.5">Output</span><span className="text-on-surface text-sm leading-snug">{step.output}</span></div></div>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="px-6 md:px-8 py-24 md:py-32 bg-surface-container-lowest">
        <div className="max-w-7xl mx-auto">
          <Reveal><div className="text-accent font-label font-bold text-xs tracking-widest uppercase mb-3">Executive use cases</div><h2 className="font-headline text-4xl md:text-6xl font-bold leading-[1.05] mb-16 max-w-3xl">Bring the work that needs to move<span className="text-primary italic">.</span></h2></Reveal>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {USE_CASES.map(([title, body], i) => <Reveal key={title} delay={Math.min(i, 3) * 70} className="h-full"><div className="bg-surface p-8 rounded-3xl soft-lift h-full"><div className="text-primary font-label font-bold text-xs tracking-widest uppercase mb-4">Use case</div><h3 className="font-headline text-xl font-bold mb-3">{title}</h3><p className="text-on-surface-variant leading-relaxed">{body}</p></div></Reveal>)}
          </div>
        </div>
      </section>

      <section className="px-6 md:px-8 py-24 md:py-32">
        <div className="max-w-7xl mx-auto"><Reveal><div className="text-accent font-label font-bold text-xs tracking-widest uppercase mb-3">Training options</div><h2 className="font-headline text-4xl md:text-6xl font-bold leading-[1.05] mb-6 max-w-3xl">Designed around the people doing the work<span className="text-primary italic">.</span></h2><p className="text-xl text-on-surface-variant max-w-3xl mb-14">Training is tailored to the people, organization, workflows, and level of AI experience. There is no one-size-fits-all curriculum.</p></Reveal><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">{OPTIONS.map((item, i) => <Reveal key={item.title} delay={i * 70} className="h-full"><div className="bg-surface-container-lowest p-8 rounded-3xl soft-lift h-full"><div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-primary-fixed mb-6"><span className="material-symbols-outlined text-accent">{item.icon}</span></div><h3 className="font-headline text-xl font-bold mb-3">{item.title}</h3><p className="text-on-surface-variant leading-relaxed text-sm">{item.body}</p></div></Reveal>)}</div></div>
      </section>

      <section className="px-6 md:px-8 py-24 md:py-32 bg-ink text-inverse-on-surface">
        <div className="max-w-7xl mx-auto"><Reveal><div className="text-primary font-label font-bold text-xs tracking-widest uppercase mb-3">Privacy, security &amp; trust</div><h2 className="font-headline text-4xl md:text-6xl font-bold leading-[1.05] mb-6 max-w-3xl">Practical AI requires practical trust<span className="text-primary italic">.</span></h2><p className="text-xl text-white/70 max-w-3xl mb-16 leading-relaxed">For family offices and other privacy-sensitive organizations, responsible AI use starts with the information, environment, access, and review process—not with a blanket claim about any particular vendor.</p></Reveal><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{TRUST.map(([title, body], i) => <Reveal key={title} delay={Math.min(i, 3) * 70} className="h-full"><div className="bg-white/5 border border-white/10 p-8 rounded-3xl h-full"><div className="inline-flex items-center justify-center w-11 h-11 rounded-2xl bg-primary/15 mb-5"><span className="material-symbols-outlined text-primary">{["visibility", "tune", "lock", "fact_check", "rule", "warning"][i]}</span></div><h3 className="font-headline text-xl font-bold mb-3">{title}</h3><p className="text-white/65 leading-relaxed text-[15px]">{body}</p></div></Reveal>)}</div></div>
      </section>

      <section className="px-6 md:px-8 py-24 md:py-32"><Reveal className="max-w-5xl mx-auto text-center"><div className="text-accent font-label font-bold text-xs tracking-widest uppercase mb-3">Who it&apos;s for</div><h2 className="font-headline text-4xl md:text-6xl font-bold leading-[1.05] mb-12">Capability, not dependency<span className="text-primary italic">.</span></h2><div className="flex flex-wrap justify-center gap-3">{AUDIENCE.map((item) => <span key={item} className="inline-flex items-center gap-2 bg-surface-container-lowest text-on-surface px-5 py-3 rounded-full soft-lift text-sm font-label font-semibold"><span className="material-symbols-outlined text-primary text-base">check</span>{item}</span>)}</div></Reveal></section>

      <section className="px-6 md:px-8 py-28 md:py-36 bg-ink text-inverse-on-surface"><Reveal className="max-w-3xl mx-auto text-center"><div className="text-primary font-label font-bold text-xs tracking-widest uppercase mb-3">Start with one real problem</div><h2 className="font-headline text-4xl md:text-6xl font-bold leading-[1.05] mb-6">Bring the work that&apos;s been stuck<span className="text-primary italic">.</span></h2><p className="text-xl md:text-2xl text-white/70 mb-10 leading-relaxed">Book a focused conversation about the work you want to improve, the information involved, and whether an AI-assisted approach would genuinely help. No generic AI lecture—just a practical next step.</p><Link href="/contact#book" className="inline-flex items-center gap-2 bg-primary text-on-primary px-8 py-4 rounded-full font-bold text-lg">Book an AI working session<span className="material-symbols-outlined">arrow_forward</span></Link></Reveal></section>
    </>
  );
}
